...
class TestFileReaderRegistration:
    """Tests for file reader registration and validation."""

    @pytest.mark.parametrize("override", [False, True])
    @pytest.mark.parametrize("register", [False, True])
    def test_file_reader_registration(self, pytester: Pytester, register: bool, override: bool) -> None:
        """Test file reader registration and per-test overriding"""
        ext = ".txt"
        rel_path, file_data = self._setup_data(pytester, ext=ext)
        if register or override:
            self._create_conftest_with_reader_registration(
    ...
    def test_file_reader_registration_in_multiple_conftest(self, pytester: Pytester) -> None:
        """Test that reader registration in the nearest conftest.py should be effective"""
        orig_dir = pytester.path
        ext = ".txt"
        rel_path, _ = self._setup_data(pytester, ext=ext)

        # Register reader in the top-level confest.py
        self._create_conftest_with_reader_registration(pytester, "io.TextIOWrapper", read_options={"mode": "rb"})
        ...
        pytester._path = p
        pytester.chdir()
        self._create_conftest_with_reader_registration(pytester, "io.BufferedReader", read_options={"mode": "rb"})
    ...
    def test_file_reader_registration_with_invalid_extension(self, pytester: Pytester) -> None:
        """Test that file extension is validated during registration"""
        rel_path, _ = self._setup_data(pytester)
        self._create_conftest_with_reader_registration(pytester, "io.TextIOWrapper", ext="foo")
        self._create_test_for_negative_cases(pytester, rel_path)
        result = pytester.runpytest_subprocess()
        assert result.ret == ExitCode.USAGE_ERROR
        assert "File extension must start with '.'" in str(result.stderr)

    def test_file_reader_registration_with_invalid_reader(self, pytester: Pytester) -> None:
        """Test that file reader is validated during registration"""
        rel_path, _ = self._setup_data(pytester)
        self._create_conftest_with_reader_registration(pytester, '"foo"')
        self._create_test_for_negative_cases(pytester, rel_path)
        result = pytester.runpytest_subprocess()
        assert result.ret == ExitCode.USAGE_ERROR
    ...
    def test_file_reader_registration_with_invalid_read_options(self, pytester: Pytester) -> None:
        ...
        rel_path, _ = self._setup_data(pytester)
        self._create_conftest_with_reader_registration(pytester, "io.TextIOWrapper", read_options={"mode": "w"})
        self._create_test_for_negative_cases(pytester, rel_path)
        result = pytester.runpytest_subprocess()
        assert result.ret == ExitCode.USAGE_ERROR
        assert "read_options: Invalid read mode" in str(result.stderr)

    def test_file_reader_registration_in_non_conftest(self, pytester: Pytester) -> None:
        """Test that reader registration is not allowed outside conftest.py"""
        ext = ".txt"
        rel_path, _ = self._setup_data(pytester, ext=ext)
        self._create_test_for_negative_cases(pytester, rel_path, register_reader=True)
        result = pytester.runpytest_subprocess()
